...
float l1_distance(float *a, float *b, int n)
{
	...
	float dis;
	int i;
	for(i=0,dis=0;i<n;i++)dis+=abs(a[i]-b[i]);
    return dis;
}
...
matrix compute_homography(match *matches, int n)
{
	...
	for (i = 0; i < 8; i++)H.data[i/H.cols][i % H.cols] = a.data[i][0];
	H.data[2][2] = 1;
    free_matrix(a);
    return H;
}
...
image combine_images(image a, image b, matrix H)
{
    ...
    for(k = 0; k < a.c; ++k){
        for(j = 0; j < a.h; ++j){
            for(i = 0; i < a.w; ++i){
                // TODO: fill in.
				set_pixel(c, i-dx, j-dy, k,bilinear_interpolate(a, i, j, k));
            }
        }
    }
	...
	for (j = 0; j <h; ++j) {
		for (i = 0; i < w; ++i) {
			point pp=project_point(H, make_point(i,j));
			//printf("%f,%f\n",pp.x,pp.y);
			pp.x+=dx;pp.y+=dy;
			if(pp.x>=0&&pp.x<b.w&&pp.y>=0&&pp.y<b.h)
			for (k = 0; k < b.c; ++k) {
				// TODO: fill in.				
				set_pixel(c,i,j, k, bilinear_interpolate(b, pp.x,pp.y, k));
			}
		}
	}
