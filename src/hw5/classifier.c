    ...
    for(i = 0; i < m.rows; ++i){
        for(j = 0; j < m.cols; ++j){
			...
			switch(a){
				...
				case LINEAR:case SOFTMAX:g=1;break;
				case RELU:g=(double)(x!=0);break;
				case LRELU:g=x<0?0.1:1;break;
			}
			d.data[i][j]*=g;
        }
    }
...
matrix forward_layer(layer *l, matrix in)
{
    ...
    matrix out = matrix_mult_matrix(l->in,LW);
...
matrix backward_layer(layer *l, matrix delta)
{
	...
	gradient_matrix(l->out,LACT,delta);
...
void update_layer(layer *l, double rate, double momentum, double decay)
{
    ...
    // save it to l->v
	matrix v=make_matrix(LW.rows,LW.cols);
	int i,j;
	for(i=0;i<v.rows;i++)for(j=0;j<v.cols;j++)
		v.data[i][j]=l->dw.data[i][j]-
			decay*LW.data[i][j]+
			momentum*l->v.data[i][j];
	free_matrix(l->v);
	l->v=v;
    // Update l->w
	matrix w=axpy_matrix(rate,l->v,LW);
	free_matrix(LW);
	LW=w;
